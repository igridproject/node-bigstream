    ...
    amqp.connect(self.url, function(err, conn) {
      ...
      conn.createChannel(function(err, ch) {
        ...
        ch.consume(REPLY_QUEUE ,
          (msg) => { console.log('reply consumee to ' + msg.properties.correlationId); ch.responseEmitter.emit(msg.properties.correlationId, JSON.parse(msg.content.toString()))},
          {noAck: true});
...
RPCCaller.prototype.call = function(req,cb){
  ...
  self.open(function(err){
    ...
    self.ch.sendToQueue(self.name, new Buffer(JSON.stringify(req)), { corr, replyTo: REPLY_QUEUE })
  });
